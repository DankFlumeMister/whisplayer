...
void main() {
  ...
  test('shuffle next visits every song once before a round resets',
      () async {
  ...
  test('onPrevious pops the random history back to the exact origin',
      () async {
        ...
        final first = container.read(playerControllerProvider).currentIndex;
        await notifier.onNext();
        final second = container.read(playerControllerProvider).currentIndex;
  ...
  test('clearQueue forgets the random history', () async {
    final container = makeContainer(
      settings: const {
